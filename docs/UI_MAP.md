...
| `/preprod/status` | control de preproduccion cards | Roles, system status, demo seed coverage and global tenant search |
...
| `/companies/:companyId/summary` | company summary panel | Issued/draft invoice counts, paid/outstanding amounts, chart and internal notifications |
...
| `/companies/:companyId/invoices/:invoiceId` | selected invoice in `#invoices` | Issue invoice, download PDF, capture PDF hash, register/filter payments, view timeline, cancel local or create corrective draft |
...
- Shared-preprod operator cards show roles, permission boundaries, service status, demo data coverage and global search.
- Company summary shows operational metrics, invoice status chart, outstanding amount and internal notifications.
- CSV exports are generated client-side for invoices, customers and audit evidence. They are Excel-compatible convenience exports, not legal evidence packs.
- Invoice and company timelines are derived from local/preprod events and visible hashes.
